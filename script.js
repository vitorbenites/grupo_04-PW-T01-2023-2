...
const mostrarCalendario = () => {
    ...
    for (let i = primeiroDiaMes; i > 0; i--) { // Listando os últimos dias do mês anterior
        liTag += `<li class="inactive">${ultimaDataUltimoMes - i + 1}</li>`;
    }

    for (let i = 1; i <= ultimaDataMes; i++) { // Listando os dias do mês atual
        // Classe ativa se é o dia atual
        let ehHoje = i === data.getDate() && mesAtual === new Date().getMonth() 
                     && anoAtual === new Date().getFullYear() ? "active" : "";
        liTag += `<li class="${ehHoje}">${i}</li>`;
    }

    for (let i = ultimoDiaMes; i < 6; i++) { // Listando os primeiros dias do próximo mês
...
prevProxIcon.forEach(icon => { // Eventos dos ícones de mes anterior e posterior
